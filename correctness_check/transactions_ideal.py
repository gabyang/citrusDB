...
class Transactions:
    ...
    def new_order_txn(self, c_id, w_id, d_id, num_items, item_number, supplier_warehouse, quantity):
        ...
        # Write output to file in array format
        output = []
        output.append([w_id, d_id, c_id, C[1], C[2], C[0]])  # Customer's identifier, lastname, credit, discount
        output.append([W[0], D[0]])  # Warehouse and District tax rate
        output.append([D[1], curr_dt])  # Order number and entry date
        output.append([num_items, ttl_amt])  # Number of items and total amount

        # Loop through items to add order details
        for item in items:
            output.append([item['ITEM_NUMBER'], item['I_NAME'], item['SUPPLIER_WAREHOUSE'],
                           item['QUANTITY'], item['OL_AMOUNT'], item['S_QUANTITY']])

        output = str(output)
        # Write the entire output array to the file
        with open('output_ideal.txt', 'a') as file:
                file.write(output + '\n')
    ...
    def popular_item_txn(self, w_id, d_id, l):
        ...
        for idx, row in enumerate(results):
            if idx == 10:
                break
            sb.append(f"{idx + 1}. Order ID: {row[0]}")
            sb.append(f"   Entry Date: {row[1]}")
            sb.append(f"   Customer: {row[2]} {row[3]} {row[4]}")
            sb.append(f"   Item Name: {row[5]}")
            sb.append(f"   Quantity: {row[6]}")
            sb.append(f"   Popularity Percentage: {row[7]:.2f}%")
            sb.append("------------------------------------------------------")

        # Join the list and print the formatted output
        output = "\n".join(sb)
        print(output)
