TXN_QUERIES = {
    "NEW_ORDER": {
        ...
        "createOrderLine": "INSERT INTO \"order-line\" (OL_O_ID, OL_D_ID, OL_W_ID, OL_NUMBER, OL_I_ID, OL_SUPPLY_W_ID, OL_DELIVERY_D, OL_QUANTITY, OL_AMOUNT, OL_DIST_INFO) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
    },
    ...
    "ORDER_STATUS": {
        ...
        "getOrderLines": "SELECT OL_SUPPLY_W_ID, OL_I_ID, OL_QUANTITY, OL_AMOUNT, OL_DELIVERY_D FROM \"order-line\" WHERE OL_W_ID = %s AND OL_D_ID = %s AND OL_O_ID = %s",  # w_id, d_id, o_id
    },

    "STOCK_LEVEL": {
        "getOId": "SELECT D_NEXT_O_ID FROM DISTRICT WHERE D_W_ID = %s AND D_ID = %s",
        "getStockCount": """
            SELECT COUNT(DISTINCT(OL_I_ID)) FROM \"order-line\", STOCK
            WHERE OL_W_ID = %s
              AND OL_D_ID = %s
              AND OL_O_ID < %s
              AND OL_O_ID >= %s
    ...
    "DELIVERY_QUERIES": {
        "getOldestUndeliveredOrder": "SELECT o_id FROM \"order\" WHERE o_w_id = %s AND o_d_id = %s AND o_carrier_id IS NULL ORDER BY o_id FOR UPDATE SKIP LOCKED LIMIT 1;",
        "updateOrderCarrierId": "UPDATE \"order\" SET o_carrier_id = %s WHERE o_w_id = %s AND o_d_id = %s AND o_id = %s",
        "updateOrderLineDeliveryDate": "UPDATE \"order-line\" SET ol_delivery_d = %s WHERE ol_w_id = %s AND ol_d_id = %s AND ol_o_id = %s",
        "updateCustomerBalanceAndDeliveryCount": """
            UPDATE customer 
            SET c_balance = COALESCE(c_balance, 0) + 
                (SELECT COALESCE(SUM(ol_amount), 0) FROM \"order-line\" 
    ...
    "RELATED_CUSTOMER_TXN_QUERIES": {
        "findRelatedCustomers": """
            ...
            related_orders AS (
                ...
                INNER JOIN \"order\" o ON ol.ol_o_id = o.o_id AND ol.ol_d_id = o.o_d_id AND ol.ol_w_id = o.o_w_id
                WHERE o.o_w_id != %s
