...
class Transactions:
    ...
    def new_order_txn(self, c_id, w_id, d_id, num_items, item_ids, supplier_warehouses, quantities):
        ...
        try:
            ...
            next_order_id = None
            self.cursor.execute(
                "SELECT D_NEXT_O_ID FROM district WHERE D_ID = %s AND D_W_ID = %s", (d_id, w_id)
            )
            ...
            self.cursor.execute(
                "SELECT I_PRICE, I_NAME FROM item WHERE I_ID = ANY(%s)", (item_ids,)
            )
            ...
            for idx, ol in enumerate(orderline_inputs):
                ...
                ol_quantity = ol[2]
                self.cursor.execute(
                    "SELECT S_QUANTITY, S_YTD, S_ORDER_CNT, S_REMOTE_CNT FROM stock WHERE S_I_ID = %s AND S_W_ID = %s",
                ...
                self.cursor.execute(
                    "UPDATE stock SET S_QUANTITY = %s, S_YTD = %s, S_ORDER_CNT = %s, S_REMOTE_CNT = %s WHERE s_w_id = %s AND s_i_id = %s",
                    (
                        next_quantity,
                        next_ytd,
                        next_order_count,
    ...
    def order_status_txn(self, c_w_id, c_d_id, c_id):
        ...
        try:
            ...
            print(
                f"Customer: {customer_info[0]} {customer_info[1]} {customer_info[2]}, Balance: {customer_info[3]}"
            )
            ...
            print(
                f"Last Order ID: {last_order[0]}, Entry Date: {last_order[1]}, Carrier ID: {last_order[2]}"
            )
            ...
            for item in order_items:
                print(
                    f"Item ID: {item[0]}, Supply Warehouse ID: {item[1]}, Quantity: {item[2]}, Amount: {item[3]}, Delivery Date: {item[4]}"
                )
    ...
    def stock_level_txn(self, w_id, d_id, threshold, num_last_orders):
        ...
        try:
            ...
            self.cursor.execute(
                "SELECT d_next_o_id FROM district WHERE d_w_id = %s AND d_id = %s", (w_id, d_id)
            )
    ...
    def popular_item_txn(self, w_id, d_id, l):
        ...
        try:
            ...
            for idx, item in enumerate(item_details):
                print(
                    f"{idx + 1}. Item ID: {item['i_id']}, Name: {item['i_name']}, Price: {item['i_price']}"
                )
                print(
                    f"   Total Quantity: {item['total_qty']}, Number of Orders: {item['num_orders']}"
                )
                print("------------------------------------------------------")
    ...
    def related_customer_txn(self, c_w_id, c_d_id, c_id):
        ...
        try:
            ...
            self.cursor.execute(
                "SELECT c_state FROM customer WHERE c_w_id = %s AND c_d_id = %s AND c_id = %s",
                (c_w_id, c_d_id, c_id),
            )
            ...
            for related_customer in related_customers:
                print(
                    f"Related Customer: Warehouse ID: {related_customer[0]}, District ID: {related_customer[1]}, Customer ID: {related_customer[2]}"
                )
